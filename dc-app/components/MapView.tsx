...
import { MapContainer, TileLayer, CircleMarker, Circle, useMap, GeoJSON, Tooltip, FeatureGroup, Pane, Marker } from 'react-leaflet';
...
export default function MapView({ features, selectedId, onSelectDC, filterState, filterCity, filterCompany, setFilterState, setFilterCity, setFilterCompany, activeAssetType, heatmapMode }: MapViewProps) {
    const router = useRouter();
    const [isLoading, setIsLoading] = useState(false);
    const [flyTarget, setFlyTarget] = useState<[number, number] | null>(null);
    const [statesGeoJson, setStatesGeoJson] = useState<any>(null);
    const [indiaOutlineGeoJson, setIndiaOutlineGeoJson] = useState<any>(null);
    ...
    const stateCenters = useMemo(() => {
        ...
        statesGeoJson.features.forEach((f: any) => {
            ...
            if (f.geometry.type === 'MultiPolygon') f.geometry.coordinates.forEach((poly: any) => poly.forEach(processRing));
            else f.geometry.coordinates.forEach(processRing);
    ...
    const activeStateGeoJson = useMemo(() => {
        ...
        const filtered = statesGeoJson.features.filter((f: any) => {
        ...
        return filtered.length ? { ...statesGeoJson, features: filtered } : null;
    ...
    const handleViewFullAssessment = (id: number) => {
        setIsLoading(true);
        router.push(`/asset/${id}?type=${activeAssetType}`);
    };
            ...
            >
                ...
                {statesGeoJson && (
                    <GeoJSON
                        ...
                        style={(feature: any) => {
                            const sn = feature.properties.NAME_1;
