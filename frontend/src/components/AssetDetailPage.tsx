...
// ─── Main component ───────────────────────────────────────────────────────────
export default function AssetDetailPage({ id, type, onBack }: { id: string; type: AssetType | null; onBack: () => void }) {
    ...
    if (!feature) return (
        <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px' }}>
            ...
            <button onClick={onBack} style={{ color: '#1e40af', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}>← Back to Registry</button>
        </div>
    );
        ...
        <div style={{ minHeight: '100vh', background: '#f1f5f9', fontFamily: 'Inter, sans-serif' }}>
            ...
            <nav style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '0 32px', height: '58px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 1px 8px rgba(0,0,0,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    <button onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#1e40af', background: 'none', border: 'none', fontWeight: 600, fontSize: '13px', cursor: 'pointer' }}>
