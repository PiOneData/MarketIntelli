...
async def _call_llm(
    prompt: str,
    max_tokens: int = 400,
    system_prompt: str = "You are a market intelligence analyst for India's renewable energy sector. Respond only with valid JSON.",
) -> str:
...
class NewsService:
    ...
    async def generate_daily_brief(self) -> str:
        ...
        if existing:
            # Invalidate cached briefs that are raw JSON (generated by old bug)
